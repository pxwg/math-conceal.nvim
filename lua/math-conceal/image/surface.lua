...
function M.clamp_cells(cols, rows)
  local cap = capacity()
...
local function code_block_layout_config(config)
  ...
  if config.code_block ~= nil then
    return config.code_block
  end
  local renderers = config.renderers or nil
...
function M.display_range(track)
...
function M.line_len(bufnr, row)
  return line_len(bufnr, row)
end
...
function M.cell_dimensions(track, width_px, height_px, config)
  ...
  return M.clamp_cells(cols, rows)
end
...
function M.show_preview(bufnr, preview, track, asset)
  ...
  local start_row, start_col, end_row, end_col = M.display_range(track)
  local cols, rows = M.clamp_cells(asset.cols, asset.rows)
  asset.cols = cols
  asset.rows = rows
  ...
  local lines = {}
  for row = 1, rows do
    lines[#lines + 1] = { { pad_text, "" }, { M.placeholder_row(row, cols), hl } }
  end
