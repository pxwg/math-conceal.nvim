...
- Non-interactive Typst display is centered on display line runs: the projection computes suppressed rows, builds runs over the remaining consecutive formula rows, and patches carrier/conceal extmarks instead of placing each formula independently.
